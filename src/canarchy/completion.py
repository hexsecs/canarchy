...
FLAGS: dict[str, list[str]] = {
    ...
    "session annotate": ["--note", "--target"] + _OUTPUT,
    "session attach": [
        "--artifact",
        "--command",
        "--derived-from",
    ...
    + _OUTPUT,
    "session bundle": ["--output"] + _OUTPUT,
    "session import": ["--name"] + _OUTPUT,
    "session load": _OUTPUT,
    "session save": ["--artifact", "--capture", "--dbc", "--interface", "--note"] + _OUTPUT,
    "session show": _OUTPUT,
    "session verify": ["--root"] + _OUTPUT,
