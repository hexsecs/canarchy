...
| `REQ-TRANSPORT-10` | Unwanted behaviour | If `filter` receives an invalid expression, the system shall return a structured error with code `INVALID_FILTER_EXPRESSION` and exit code 2, and the JSON error envelope shall not carry a `frames` / `frame_count` data block that could be mistaken for a successful zero-match. |
| `REQ-TRANSPORT-15` | Ubiquitous | Filter expression operands for `id==`, `pgn==`, and `sa==` shall accept decimal, `0x`-prefixed hex, and bare hex values, and all filter operators (`==`, `>`, `~=`) shall tolerate whitespace around them; `sa==` shall match only the J1939 source address of extended frames. |
