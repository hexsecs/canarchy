...
| `REQ-TRANSPORT-15` | Ubiquitous | Filter expression operands for `id==` and `pgn==` shall accept decimal, `0x`-prefixed hex, and bare hex values, and all filter operators (`==`, `>`, `~=`) shall tolerate whitespace around them. |
