...
Phase 1 is implemented: the CLI now consumes a decoder abstraction. Phase 2 is partially implemented: file-backed `j1939 decode` and `j1939 pgn` route through that abstraction, and the underlying identifier and DTC parsing paths use `can-j1939` helpers. Phase 3 is partially implemented: `j1939 spn`, `j1939 tp`, and `j1939 dm1` now execute through a `can-j1939`-backed decoder adapter instead of thin CLI wrappers over the legacy helper functions. Phase 4 is partially implemented: `j1939 decode`, `j1939 pgn`, and `j1939 spn` accept optional DBC enrichment through `--dbc <path|provider-ref>` or a configured default J1939 DBC path. `j1939 spn` can now resolve non-curated SPNs from DBC signal SPN metadata when the requested SPN is not present in the starter map.
