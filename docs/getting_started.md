...
!!! tip "Acquire quickly, analyze bounded"
    `datasets replay` is **real-time by default** (`--rate 1.0`), and `datasets fetch` records provenance only — it does not download frames. To pull a fast, bounded slice raise `--rate` and cap with `--max-frames`/`--max-seconds` (note: `datasets replay` uses `--max-seconds`, not `--seconds`). For per-frame decoders on large files, run `capture-info` first and pass its `suggested_max_frames`/`suggested_seconds` to the command's `--max-frames`/`--seconds`. See [Working with large captures](troubleshooting.md#working-with-large-captures) and [Acquiring dataset data](troubleshooting.md#acquiring-dataset-data).
