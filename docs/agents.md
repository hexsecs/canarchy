...
The TUI is for human inspection, not agent automation. Its command entry uses the shared CLI executor; non-event results and errors appear in a read-only result view (`F2` switches back to the panes, `Esc` closes the result). Agents should use CLI JSON or MCP tools for machine-readable results.
