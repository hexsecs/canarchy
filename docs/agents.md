...
The TUI is for human inspection, not agent automation. `Alt+1`–`Alt+5` switch the responsive Traffic, Signals, J1939, UDS, and Findings workspaces; `F3` expands activity, and `Enter` drills into a selected row on narrow terminals. Its command entry uses the shared CLI executor; non-event results and errors appear in a read-only result view (`F2` switches back to the workspaces, `Esc` closes the result). Agents should use CLI JSON or MCP tools for machine-readable results.
