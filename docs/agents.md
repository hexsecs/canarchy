...
The TUI is for human inspection, not agent automation. `Alt+1`–`Alt+5` switch the responsive Traffic, Signals, J1939, UDS, and Findings workspaces. Traffic defaults to a typed per-bus identifier summary; `v` switches to the chronological event log, `F4` freezes inspection or returns live, and `Enter` drills into a selected row on narrow terminals. `/filter traffic` accepts the CLI filter atoms `id==`, `pgn==`, and `sa==` (J1939 source address, extended frames only), while plain text still performs substring filtering; invalid typed filters and unknown sort fields are reported. The `sa==` atom also works in non-interactive `canarchy filter` with unchanged output schemas. The TUI command entry uses the shared CLI executor; non-event results and errors appear in a read-only result view (`F2` switches back to the workspaces, `Esc` closes the result). Agents should use CLI JSON or MCP tools for machine-readable results.

DBC `decode` results populate Traffic from `decoded_message` embedded frames and correlated signals. Active Traffic filters also constrain live inspector follow; `F4` returns from historical inspection to the newest visible log event.
