...
* Exposed the **plugin registry** via a `canarchy plugins {list,info,enable,disable}` CLI subcommand tree and two MCP tools (`plugins_list`, `plugins_info`). `plugins list` enumerates every discovered plugin (built-in or third-party entry-point) with its name, kind (`processor`, `sink`, `input_adapter`), API version, and enabled status. `plugins info <name>` shows per-plugin detail including supported formats or extensions. `plugins enable <name>` and `plugins disable <name>` persist the toggle to `~/.canarchy/config.toml` under `[plugins] disabled = [...]` without affecting the current session. The CLI returns the canonical envelope on every path; `PLUGIN_NOT_FOUND` is raised for unknown names. 13 new tests in `tests/test_cli.py` and 3 in `tests/test_mcp.py` cover the list/info/enable/disable paths, unknown-name errors, round-trip enable→disable toggling, and MCP tool coverage. The `Extensibility via plugins / modules` row in `docs/feature-matrix.md` moves from `Planned` to `Yes`. Closes #317.
